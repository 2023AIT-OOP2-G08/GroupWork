...
class BookselfTest(BaseScreen):
    ...
    def create_widgets(self):
        ...
        def on_delete_button_click(item, index): # 本棚から削除
            ...
            BookselfTest.input_data = Processor.remove_index_elements(SHELF_JSON_PATH, BookselfTest.input_data, index - 1)
        ...
        def update_table():
            ...
            for widget in bookself_table_frame.winfo_children(): # table_frame内のすべての子ウィジェットを破棄
                widget.destroy()
            BookselfTest.photos = [] # 検索結果の画像を初期化
            ...
            for i, item in enumerate(bookself_data, start=1): # 表の要素配置(row=3から)
                ...
                BookselfTest.photos.append(photo)  # 画像をリストに追加
...
if __name__ == '__main__':
    ...
    book_self_screen = BookselfTest(root) # クラスをインスタンス化
